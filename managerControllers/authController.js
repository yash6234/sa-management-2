...
const { encryptData, decryptData ,verifyRecaptcha,logger} = require("../utils/enc_dec_m");
...
const Login = async (req, res) => {
  try {
    logger.info("Manager Login request received");
    ...
    if (!user.isVerified) {
      logger.warn("Manager Account not verified");
      return res.status(400).json({ message: 'Manager Account not verified' });
    }
    ...
    logger.info(`Manager authenticated successfully - ${user}`);
    ...
    logger.info(`User Verified and OTP Sent to Manager on ${user.email} and OTP : ${user.otp}`);
...
const LoginApp = async (req, res) => {
  try {
    logger.info("Manager Login request received (From Application)");
    const decryptedData = decryptData(req.params.data);
    logger.info(`Decrypted login data - ${decryptedData.email} - ${decryptedData.password}`);
...
const VerifyOTP = async (req,res)=>{
    try {
        logger.info("Manager OTP Verify request received");
        const decryptedData = decryptData(req.params.data);
        ...
        const token = jwt.sign({ id: user._id,mobile_no:user.mobile_no,email:user.email,expiry_at:hdt.expiry_at }, process.env.MANAGER_JWT_SECRET);
...
const VerifyOTPApp = async (req,res)=>{
    try {
        logger.info("Manager OTP Verify request received (From Application)");
        const decryptedData = decryptData(req.params.data);
